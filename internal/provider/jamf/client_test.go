...
func TestStatusCodePreservesJamfAPIStatus(t *testing.T) {
	...
	client := newClient(server.URL, "fixture-client", "fixture-secret", server.Client(), time.Now)
	err := client.RenameComputer(context.Background(), "computer-1", "FIXTURE-NAME")
	if err == nil {
		t.Fatal("RenameComputer() error = nil, want API error")
	}
	status, ok := StatusCode(err)
	if !ok || status != http.StatusForbidden {
		t.Errorf("StatusCode() = %d, %t, want 403, true", status, ok)
	}
	if strings.Contains(err.Error(), "fixture-secret") {
		t.Error("error contains client secret")
	}
}
