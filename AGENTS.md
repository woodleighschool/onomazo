...
- Planning consumes complete provider snapshots and remains read-only. Applying a plan records rename intentions and sends the required MDM commands.
- Naming policy and collision resolution are deterministic. Provider packages own transport; the planner owns desired names and intentions.
- YAML is user-authored policy. Environment substitution, decoding, derivation, validation, and generated schema remain one configuration contract.
