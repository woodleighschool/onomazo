...
func TestFailedReconciliationRetainsMeasuredResults(t *testing.T) {
	...
	text := rows.String()
	for _, want := range []string{"✗ Device names  failed", "    ✓ Fetching inventory", "3/3 sources 100%", "    ✗ Planning changes"} {
		if !strings.Contains(text, want) {
			t.Fatalf("missing %q: %s", want, text)
		}
	}
	if strings.Count(text, "Device names") != 1 || strings.Count(text, "Fetching inventory") != 1 || strings.Contains(text, "(0s)") || strings.Contains(text, "·") {
